...
export async function GET(request: Request) {
    ...
    if (!parsed.success) {
        return NextResponse.json(
            { error: "Parameter media tidak valid." },
            { status: 400 },
        );
    }

    const result = await getAdminMediaLibrary({
...
export async function POST(request: Request) {
    ...
    try {
        ...
        return NextResponse.json(
            { error: "Request upload tidak valid." },
            { status: 400 },
        );
    }

    const parsedModule = mediaModuleSchema.safeParse(formData.get("module"));

    if (!parsedModule.success) {
        return NextResponse.json(
            { error: "Module media tidak valid." },
            { status: 400 },
        );
    }
    ...
    if (!(fileValue instanceof File)) {
        return NextResponse.json(
            { error: "File image wajib dipilih." },
            { status: 400 },
        );
    }

    try {
        const validated = await validateImageUpload(fileValue);
        ...
        try {
            const media = await prisma.mediaUpload.create({
                data: {
                    ...
                    // filePath menyimpan Cloudinary public_id untuk keperluan deletion
            ...
            return NextResponse.json(
                { item: serializeMediaItem(media) },
                { status: 201 },
            );
        } catch (databaseError) {
            // Rollback: hapus dari Cloudinary jika DB gagal
            try {
                await deleteImageFromCloudinary(cloudinaryResult.publicId);
            } catch (cleanupError) {
                console.error(
                    "Gagal menghapus file Cloudinary setelah database error:",
                    cleanupError,
                );
            }

            console.error("Gagal menyimpan metadata media:", databaseError);

            return NextResponse.json(
                { error: "File tidak dapat disimpan ke database." },
                { status: 500 },
            );
        }
    } catch (error) {
        ...
        return NextResponse.json(
            { error: "Upload gagal karena kesalahan server." },
            { status: 500 },
        );
    }
}